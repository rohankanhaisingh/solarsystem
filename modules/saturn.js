...
const image = ce("img.img-saturn"); image.src = './images/saturn.png';

export class Saturn {
    constructor(radius) {
        this.radius = radius;
        this.x = canvas.width / 2 - 510;
        this.y = canvas.height / 2 - 510;
        this.tickSpeed = 0.01;
        this.tick = 0;
    }
    draw() {
        ctx.beginPath();
        ctx.arc(canvas.width / 2, canvas.height / 2, 590 + (this.radius), 0, 2 * Math.PI);
        ctx.strokeStyle = '#696969';
        ctx.stroke();


        ctx.beginPath();
        ctx.drawImage(image, this.x, this.y, this.radius + 50, this.radius);
    }
    update() {
