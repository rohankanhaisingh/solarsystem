...
const image = ce("img.img-earth"); image.src = './images/earth.png';

export class Earth {
    constructor(radius) {
        this.radius = radius;
        this.x = canvas.width / 2 - 210 - (this.radius / 2);
        this.y = canvas.height / 2 - 210 - (this.radius / 2);
        this.tick = 0;
        this.tickSpeed = -0.1;
    }
    draw() {
        ctx.beginPath();
        ctx.arc(canvas.width / 2, canvas.height / 2, 260 + (this.radius / 2), 0, 2 * Math.PI);
